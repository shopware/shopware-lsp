...
func (p *AdminReferenceProvider) declarationLocations(
	...
	addDeclaration := func(
		...
		if path != "" && (nameRange.Declaration || nameRange.Identifier) {
			result = append(result, adminUsageLocation(path, nameRange))
			return
		}
		add(path, line)
	}
	...
	case admin.AdminSymbolComponent:
		values, err := p.index.GetComponent(target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
			add(value.FilePath, value.Line)
		}
	case admin.AdminSymbolService:
		values, err := p.index.GetService(target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
	...
	case admin.AdminSymbolStore:
		values, err := p.index.GetStore(target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
	...
	case admin.AdminSymbolStoreMember:
		values, err := p.index.GetStore(target.Owner)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
			if member, found := value.Member(target.Name); found {
				add(member.FilePath, member.Line)
			}
		}
	case admin.AdminSymbolPrivilege:
		values, err := p.index.GetPrivilege(target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
	...
	case admin.AdminSymbolMixin:
		values, err := p.index.GetMixin(target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
		...
		if target.Owner != "" {
			...
			for _, component := range components {
				if local, found := component.LocalDirective(target.Name); found {
					add(local.FilePath, local.Line)
				}
			}
			break
		}
		values, err := p.index.GetDirective(target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
	...
	case admin.AdminSymbolFilter:
		values, err := p.index.GetFilter(target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
	...
	case admin.AdminSymbolCMSElement, admin.AdminSymbolCMSBlock:
		...
		values, err := p.index.GetCMSRegistration(kind, target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
	...
	case admin.AdminSymbolModule:
		values, err := p.index.GetModule(target.Name)
		if err != nil {
			return nil, err
		}
		for _, value := range values {
	...
	case admin.AdminSymbolModuleRoute:
		module, route, err := p.index.GetModuleRoute(target.Name)
		if err != nil {
			return nil, err
		}
		if module != nil && route != nil {
			add(module.FilePath, route.Line)
		}
	case admin.AdminSymbolEventBusEvent:
		event, found, err := p.index.ResolveShopwareEventBusEvent(
			target.Name, "",
		)
		if err != nil {
			return nil, err
		}
		if !found || event.DefinitionPath == "" {
			break
		}
		if event.DefinitionRange.Declaration ||
			event.DefinitionRange.Identifier {
			result = append(result, protocol.Location{
		...
		add(event.DefinitionPath, event.DefinitionLine)
	case admin.AdminSymbolComponentProp,
		...
		for _, component := range components {
			if target.Kind == admin.AdminSymbolComponentProp {
				...
				addDeclaration(target.Owner, prop.Line, prop.NameRange)
				continue
			}
			if target.Kind == admin.AdminSymbolComponentEvent {
				event, _ := component.ComponentEvent(target.Name)
				addDeclaration(target.Owner, event.Line, event.NameRange)
				continue
			}
			if slot, found := component.ComponentSlot(target.Name); found {
				addDeclaration(target.Owner, slot.Line, slot.NameRange)
			}
		}
	}
	return result, nil
